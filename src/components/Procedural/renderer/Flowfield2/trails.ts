...
export function setupTrailsResources(
  ...
  const trailsModule = device.createShaderModule({
    code: `${commonWgsl}\n${trailsWgsl}`,
  });
...
export function renderTrails(
  ...
  trails: {
    ...
    addPipeline: GPURenderPipeline;
    addBindGroupLayout: GPUBindGroupLayout;
  ...
  const fadeBind = device.createBindGroup({
    layout: trails.fadePipeline.getBindGroupLayout(0),
    entries: [
  ...
  const fadeAttachment: GPURenderPassColorAttachment = {
    view: dstView,
    loadOp: "clear",
    storeOp: "store",
    clearValue: [0, 0, 0, 0],
  };
  const fadeDesc: GPURenderPassDescriptor = {
    colorAttachments: [fadeAttachment],
  };
  ...
  const addBindGroup = device.createBindGroup({
    layout: trails.addBindGroupLayout,
    entries: [
      { binding: 0, resource: trails.sampler },
      { binding: 1, resource: particleTexture.createView() },
      { binding: 2, resource: { buffer: trails.colorBuffer } },
      { binding: 3, resource: { buffer: dataBuffer } },
    ],
  });
  const addAttachment: GPURenderPassColorAttachment = {
    view: dstView,
    loadOp: "load",
    storeOp: "store",
  };
  const addDesc: GPURenderPassDescriptor = {
    colorAttachments: [addAttachment],
  };
