export async function setupGreenNoiseRenderer(
  ...
  const module = device.createShaderModule({
    ...
    code: /* wgsl */ `      
      ...
      @fragment fn fs(@builtin(position) coord: vec4<f32>) -> @location(0) vec4f {
        ...
        let darkGreen = vec3f(0.1, 0.3, 0.1);
        let lightGreen = vec3f(0.5, 0.9, 0.4);
        let greenColor = mix(darkGreen, lightGreen, n);
        
        return vec4<f32>(greenColor, 1.0);
      }
    `,
  });

  const pipeline = device.createRenderPipeline({
    label: "green noise pipeline",
    layout: "auto",
    vertex: {
      module,
    },
  ...
  return {
    ...
    async update(
      ...
      const encoder = device.createCommandEncoder({ label: "green encoder" });
