  ...
  const module = device.createShaderModule({
    ...
    code: /* wgsl */ `      
      ...
      @fragment fn fs(@builtin(position) coord: vec4<f32>) -> @location(0) vec4f {
        ...
        // 3D perspective: y-axis becomes elevation, treat screen top as higher viewpoint
        // This creates a pseudo-3D isometric effect
        let horizonLine = data.height * 0.35; // horizon is 35% from top
        let screenDistFromHorizon = coord.y - horizonLine;
        let perspectiveStretch = 1.0 + screenDistFromHorizon / (data.height * 0.4); // stretch factor
        
        // Scale X by perspective for isometric appearance
        let perspectiveX = x * perspectiveStretch;
        
        // Six layers with specific purposes
        let layer1 = openSimplex3d(perspectiveX * 0.005, y * 0.005, data.z);   // Global ocean/continent distribution
        let layer2 = openSimplex3d(perspectiveX * 0.015, y * 0.015, data.z);   // Macro-biomes (desert, forest, ice, tropical)
        let layer3 = openSimplex3d(perspectiveX * 0.05, y * 0.05, data.z);     // Detailed height (hills, valleys)
        let layer4 = openSimplex3d(perspectiveX * 0.08, y * 0.08, data.z);     // Earth type (affects mountain colors only)
        let layer5 = openSimplex3d(perspectiveX * 0.02, y * 0.02, data.z);     // Inland lakes and water bodies
        let layer6 = openSimplex3d(perspectiveX * 0.3, y * 0.3, data.z);       // Fine detail height variation
        ...
        let mountainHeightMod = openSimplex3d(perspectiveX * 0.01, y * 0.01, data.z); // Even larger scale for mountain ranges
        ...
        let biomeSmooth1 = openSimplex3d((perspectiveX + 5.0) * 0.015, y * 0.015, data.z);
        let biomeSmooth2 = openSimplex3d(perspectiveX * 0.015, (y + 5.0) * 0.015, data.z);
        ...
        // Shadow/shading based on proximity to horizon for depth perception
        let depthShading = mix(0.5, 1.0, smoothstep(horizonLine - 100.0, horizonLine + 100.0, coord.y));
