export async function setupOpenSimplex2SRenderer(
  ...
  const module = device.createShaderModule({
    ...
    code: /* wgsl */ `
      ...
      // OpenSimplex2S (KdotJPG) port to WGSL
      ...
      fn openSimplex2SDerivativesPart(X: vec3f) -> vec4f {
        ...
        // derivative = -8.0 * mat4x3(d1,d2,d3,d4) * (aa * a * extrapolations) + mat4x3(g1,g2,g3,g4) * aaaa;
        let term1 = vec3f(0.0);
        let term2 = vec3f(0.0);
        // For fragments we only need the scalar value (w); return derivative zero.
      ...
      @fragment fn fs(@builtin(position) coord: vec4<f32>) -> @location(0) vec4f {
        ...
        let n4 = openSimplex2SDerivatives_ImproveXY(coords);
        let n = n4.w;
        let v = n * 0.5 + 0.5;
  ...
  const renderPassDescriptor: GPURenderPassDescriptor = {
    label: "opensimplex2s renderPass",
  ...
  return {
    ...
    async update(time: DOMHighResTimeStamp, data?: { x?: number; y?: number }) {
      ...
      const encoder = device.createCommandEncoder({ label: "opensimplex2s encoder" });
