class Constantly {
  ...
  constructor(callsPerSecond = 0) {
    ...
    this.interval =
      this.callsPerSecond <= 50 && this.callsPerSecond !== 0 ? 20 : 0;
  ...
  each(callback: Function) {
    ...
    const batch = () => {
      ...
      if (!this._isDestroyed) this.timer = setTimeout(batch, this.interval);
    };
...
function constantly(callback: Function, callsPerSecond: number) {
